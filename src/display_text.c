...
static size_t    s_char_cap = 0;

static void draw_char_5x7(esp_lcd_panel_handle_t panel,
                          int x, int y, char c,
    ...
    for (int row = 0; row < 7; ++row) {
        for (int col = 0; col < 6; ++col) { // 5 data cols + 1 space
            ...
            for (int sy = 0; sy < scale; ++sy) {
                uint16_t *dst = buf + (row*scale + sy) * cell_w + (col*scale);
                          ...
                          int scale)
{
    ...
    for (const char *p = s; *p; ++p) {
        if (*p == '\n') { y += 7 * scale; x = 0; continue; }
        draw_char_5x7(panel, x, y, *p, fg565, bg565, scale);
        x += cell_w;
    }
}